// Activity: Human Nutrition — Age 9 (Band B · CAPS Gr 4)
//   Plan (predict) → Sort 6 foods into food groups → Conclude → Apply
...
type Group = 'energy' | 'build' | 'protect';

interface Food { id: string; emoji: string; name: string; group: Group; }
const FOODS: Food[] = [
  { id: 'bread', emoji: '🍞', name: 'Bread',  group: 'energy' },
  { id: 'rice',  emoji: '🍚', name: 'Rice',   group: 'energy' },
  { id: 'egg',   emoji: '🥚', name: 'Egg',    group: 'build' },
  { id: 'beans', emoji: '🫘', name: 'Beans',  group: 'build' },
  { id: 'apple', emoji: '🍎', name: 'Apple',  group: 'protect' },
  { id: 'carrot',emoji: '🥕', name: 'Carrot', group: 'protect' },
];
const GROUPS: { id: Group; label: string; emoji: string }[] = [
  { id: 'energy',  label: 'Energy',     emoji: '⚡' },
  { id: 'build',   label: 'Body-build', emoji: '💪' },
  { id: 'protect', label: 'Protecting', emoji: '🛡️' },
];

const OBJECTIVE_ID = 'll.nutrition';
const BADGE = 'Healthy Eater Badge 🍎';

const PLAN_VOICE = "A balanced plate has different food groups. Which food gives you energy to run — bread, or an apple? Predict!";
const TEST_VOICE = "Sort each food into its group: energy, body-building, or protecting.";
const CONCLUDE_VOICE = "Finish your conclusion. Choose a word for each blank.";
const CARD_VOICE = "You earned the Healthy Eater Badge! At dinner: look at your plate. Name one energy food and one food that protects your body.";
...
export default function NutritionActivity({ onComplete, onExit }: Props) {
  ...
  function predict() { setPredicted(true); speak("Let's sort some foods into their groups!", 0.9, () => { setPhase('test'); setTimeout(() => speak(TEST_VOICE), 300); }); }

  function sort(g: Group) {
    if (feedback !== 'idle') return;
    const correct = g === cur.group;
    setFeedback(correct ? 'right' : 'wrong');
    const grpLabel = GROUPS.find(x => x.id === cur.group)!.label;
    speak(correct ? `Yes! ${cur.name} is an ${grpLabel} food.` : `Actually, ${cur.name} is an ${grpLabel} food.`, 0.92);
    setTimeout(() => {
      setFeedback('idle');
  ...
  function check() {
    ...
    if (blank1 === 'energy' && blank2 === 'protect') {
      setResult('correct'); awardBadge(OBJECTIVE_ID, BADGE);
      speak("Correct! Bread gives us energy, and fruit and vegetables protect our body. A balanced plate has all the groups. You are a Healthy Eater!", 0.85, () => setTimeout(() => setPhase('card'), 400));
    } else { setResult('wrong'); speak("Think about what bread does, and what fruit and veg do. Try again.", 0.9); setTimeout(() => setResult('idle'), 1600); }
  }
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'plan' && (
          <motion.div key="plan" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full p-6 items-center justify-center gap-5">
            ...
            <div className="bg-white rounded-3xl p-5 border-2 border-orange-100 shadow-md text-center w-full max-w-sm">
              ...
              <p className="font-display font-black text-lg text-gray-800 leading-snug">Which food gives you energy to run?</p>
            ...
            {!predicted ? (
              <div className="flex gap-3 w-full max-w-sm">
                <motion.button whileTap={{ scale: 0.92 }} onClick={predict} className="flex-1 p-4 bg-white border-2 border-orange-200 rounded-3xl active:scale-95"><span className="text-3xl">🍞</span><p className="font-black text-orange-700 text-[11px] uppercase mt-1">Bread</p></motion.button>
                <motion.button whileTap={{ scale: 0.92 }} onClick={predict} className="flex-1 p-4 bg-white border-2 border-orange-200 rounded-3xl active:scale-95"><span className="text-3xl">🍎</span><p className="font-black text-orange-700 text-[11px] uppercase mt-1">Apple</p></motion.button>
              </div>
        ...
        {phase === 'test' && !done && (
          <motion.div key="test" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full p-5 items-center justify-center gap-5">
            ...
            <p className="text-xs text-orange-500 font-bold">Which group does it belong to?</p>
            <div className="flex gap-2 w-full max-w-xs">
              {GROUPS.map(g => (
                <motion.button key={g.id} whileTap={{ scale: 0.92 }} onClick={() => sort(g.id)} disabled={feedback !== 'idle'} className="flex-1 py-3 bg-white border-2 border-orange-200 rounded-2xl font-black text-orange-700 text-[10px] uppercase active:scale-95 flex flex-col items-center gap-1">
          ...
          <motion.div key="conclude" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex flex-col h-full p-5 justify-center gap-4">
            ...
            <motion.div animate={result === 'wrong' ? { x: [-8, 8, -8, 8, 0] } : {}} transition={{ duration: 0.4 }} className="bg-white rounded-3xl p-5 border-2 border-orange-100 shadow-md">
              <p className="text-lg text-gray-800 font-bold leading-relaxed text-center">
                ...
                <span className={`inline-block min-w-[60px] px-2 py-0.5 rounded-lg font-black border-b-4 ${blank2 ? 'bg-orange-100 border-orange-400 text-orange-700' : 'border-orange-200 text-orange-300'}`}>{blank2 === 'protect' ? 'protect' : blank2 === 'weaken' ? 'weaken' : '____'}</span>{' '}
                our body.
              </p>
            </motion.div>
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Blank 1 — bread gives us…</p>
              <div className="flex gap-2">{['energy', 'bones'].map(w => (<button key={w} onClick={() => setBlank1(w)} className={`flex-1 py-2 rounded-2xl font-black text-sm border-2 active:scale-95 capitalize ${blank1 === w ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-orange-700 border-orange-200'}`}>{w}</button>))}</div>
            </div>
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Blank 2 — fruit & veg…</p>
              <div className="flex gap-2">{['protect', 'weaken'].map(w => (<button key={w} onClick={() => setBlank2(w)} className={`flex-1 py-2 rounded-2xl font-black text-sm border-2 active:scale-95 capitalize ${blank2 === w ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-orange-700 border-orange-200'}`}>{w}</button>))}</div>
            </div>
  ...
  return (
    <motion.div key="card" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col h-full p-6 items-center">
      ...
      <motion.div initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: 'spring', damping: 9 }} className="flex flex-col items-center mb-3">
        ...
        <p className="font-display font-black text-orange-700 text-sm">Healthy Eater Badge!</p>
      </motion.div>
      <div className="bg-white rounded-3xl p-5 shadow-xl border-2 border-orange-200 w-full max-w-xs mb-5 text-center">
        ...
        <p className="font-black text-gray-700 text-base leading-snug">At dinner: look at your plate. Name one food that gives energy and one that protects your body. Is it balanced?</p>
      </div>
