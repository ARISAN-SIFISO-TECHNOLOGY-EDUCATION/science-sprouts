// Activity: Biodiversity & Classification — Age 12 (Band C12 · CAPS Gr 7)
//   Plan (predict) → Classify living things into kingdoms → Conclude → Apply
...
type Group = 'animal' | 'plant' | 'fungus';

interface Org { id: string; emoji: string; name: string; group: Group; }
const ORGS: Org[] = [
  { id: 'dog',     emoji: '🐕', name: 'Dog',       group: 'animal' },
  { id: 'butterfly', emoji: '🦋', name: 'Butterfly', group: 'animal' },
  { id: 'fern',    emoji: '🌿', name: 'Fern',      group: 'plant' },
  { id: 'flower',  emoji: '🌷', name: 'Tulip',     group: 'plant' },
  { id: 'mushroom', emoji: '🍄', name: 'Mushroom', group: 'fungus' },
  { id: 'toadstool', emoji: '🍄‍🟫', name: 'Toadstool', group: 'fungus' },
];
const GROUPS: { id: Group; label: string; emoji: string }[] = [
  { id: 'animal', label: 'Animal', emoji: '🐾' },
  { id: 'plant',  label: 'Plant',  emoji: '🌱' },
  { id: 'fungus', label: 'Fungus', emoji: '🍄' },
];

const OBJECTIVE_ID = 'll.classification';
const BADGE = 'Classifier Badge 🔬';

const PLAN_VOICE = "There are millions of kinds of living things — that is biodiversity. Scientists sort them into big groups called kingdoms: animals, plants, and fungi. Is a mushroom a plant, or a fungus? Predict!";
const TEST_VOICE = "Sort each living thing into its kingdom: animal, plant, or fungus.";
const CONCLUDE_VOICE = "Finish your conclusion. Choose a word for each blank.";
const CARD_VOICE = "You earned the Classifier Badge! Mushrooms are not plants — they cannot make their own food, so they have their own kingdom. Look for a mushroom or mould. It belongs to the fungus kingdom!";
...
export default function ClassificationActivity({ onComplete, onExit }: Props) {
  ...
  function predict() { setPredicted(true); speak("Let's sort living things into kingdoms!", 0.9, () => { setPhase('test'); setTimeout(() => speak(TEST_VOICE), 300); }); }
  ...
  function sort(g: Group) {
    ...
    speak(correct ? `Correct! A ${cur.name.toLowerCase()} is in the ${grp} kingdom.` : `Actually, a ${cur.name.toLowerCase()} is in the ${grp} kingdom.`, 0.92);
    setTimeout(() => {
      setFeedback('idle');
  ...
  function check() {
    ...
    if (blank1 === 'animal' && blank2 === 'fungus') {
      setResult('correct'); awardBadge(OBJECTIVE_ID, BADGE);
      speak("Correct! An animal that moves and eats belongs to the animal kingdom. A mushroom cannot make its own food and is not a plant — it belongs to the fungus kingdom. You are a Classifier!", 0.85, () => setTimeout(() => setPhase('card'), 400));
    } else { setResult('wrong'); speak("A dog is an animal. A mushroom is a fungus. Try again.", 0.9); setTimeout(() => setResult('idle'), 1600); }
  }
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'plan' && (
          <motion.div key="plan" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full p-6 items-center justify-center gap-5">
            ...
            <div className="bg-white rounded-3xl p-5 border-2 border-lime-100 shadow-md text-center w-full max-w-sm">
              ...
              <p className="font-display font-black text-lg text-gray-800 leading-snug">Is a mushroom a plant, or a fungus?</p>
            ...
            {!predicted ? (
              <div className="flex gap-3 w-full max-w-sm">
                <motion.button whileTap={{ scale: 0.92 }} onClick={predict} className="flex-1 p-4 bg-white border-2 border-lime-200 rounded-3xl active:scale-95"><span className="text-3xl">🌱</span><p className="font-black text-lime-700 text-[11px] uppercase mt-1">Plant</p></motion.button>
                <motion.button whileTap={{ scale: 0.92 }} onClick={predict} className="flex-1 p-4 bg-white border-2 border-lime-200 rounded-3xl active:scale-95"><span className="text-3xl">🍄</span><p className="font-black text-lime-700 text-[11px] uppercase mt-1">Fungus</p></motion.button>
              </div>
        ...
        {phase === 'test' && !done && (
          <motion.div key="test" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full p-5 items-center justify-center gap-5">
            <button onClick={onExit} className="self-start text-lime-500 text-sm font-bold">← Back</button>
            <p className="text-center text-xs text-lime-500 font-black uppercase">Living thing {idx + 1} of {ORGS.length}</p>
            ...
            <p className="text-xs text-lime-600 font-bold">Which kingdom?</p>
          ...
          <motion.div key="conclude" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex flex-col h-full p-5 justify-center gap-4">
            ...
            <motion.div animate={result === 'wrong' ? { x: [-8, 8, -8, 8, 0] } : {}} transition={{ duration: 0.4 }} className="bg-white rounded-3xl p-5 border-2 border-lime-100 shadow-md">
              <p className="text-lg text-gray-800 font-bold leading-relaxed text-center">
                A dog that moves and eats is in the{' '}
            ...
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Blank 1 — a dog is an…</p>
              <div className="flex gap-2">{['animal', 'plant'].map(w => (<button key={w} onClick={() => setBlank1(w)} className={`flex-1 py-2 rounded-2xl font-black text-sm border-2 active:scale-95 capitalize ${blank1 === w ? 'bg-lime-600 text-white border-lime-600' : 'bg-white text-lime-700 border-lime-200'}`}>{w}</button>))}</div>
            </div>
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Blank 2 — a mushroom is a…</p>
              <div className="flex gap-2">{['fungus', 'plant'].map(w => (<button key={w} onClick={() => setBlank2(w)} className={`flex-1 py-2 rounded-2xl font-black text-sm border-2 active:scale-95 capitalize ${blank2 === w ? 'bg-lime-600 text-white border-lime-600' : 'bg-white text-lime-700 border-lime-200'}`}>{w}</button>))}</div>
            </div>
  ...
  return (
    <motion.div key="card" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col h-full p-6 items-center">
      ...
      <motion.div initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: 'spring', damping: 9 }} className="flex flex-col items-center mb-3">
        ...
        <p className="font-display font-black text-lime-700 text-sm">Classifier Badge!</p>
      </motion.div>
      <div className="bg-white rounded-3xl p-5 shadow-xl border-2 border-lime-200 w-full max-w-xs mb-5 text-center">
        <p className="text-5xl mb-3">🐾🌱🍄</p>
        <p className="font-black text-gray-700 text-base leading-snug">Mushrooms aren't plants — they can't make their own food, so they get their own kingdom. Find a mushroom or mould: that's a fungus!</p>
      </div>
